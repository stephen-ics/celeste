...
func TestFunctionObject(t *testing.T) {
	...
	if len(fn.Parameters) != 1 {
		t.Fatalf("function has wrong parameter length. Should be %d, got='%d', Parameters=%+v", 1, len(fn.Parameters), fn.Parameters)
	}
	...
	for _, tt := range tests {
		...
		case string:
			...
			if !ok {
				...
				continue // Stops current for loop iteration and starts the next
			}
...
func TestArrayIndexExpressions(t *testing.T) {
	tests := []struct {
		...
		{"let myArray = [1, 2, 3]; let i = myArray[0]; myArray[1]", 2},
		{"[1, 2, 3][3]", nil},
		{"[1, 2, 3][-1]", nil},
	}
